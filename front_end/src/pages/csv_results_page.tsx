...
const handleExport = async () => {
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col pt-24 transition-colors duration-300 ${
      isDarkMode ? 'bg-black text-white' : 'bg-white text-gray-900'
    }`}>
      <div className={`flex flex-col items-center justify-between px-8 py-8 w-full mx-auto min-h-[calc(100vh-90px)] ${
        isDarkMode ? 'bg-black' : 'bg-white'
      }`}>
        <div className={`flex flex-col w-full gap-8 flex-1 max-w-5xl ${
          isDarkMode ? 'bg-black' : 'bg-white'
        }`}>
          ...
          <div className="w-full flex justify-center">
            <div
              ...
              className={`relative w-full max-w-4xl ${
                isFullscreen
                  ? `fixed inset-0 z-30 flex justify-center items-start overflow-auto pt-16 px-4 ${
                      isDarkMode ? 'bg-black text-white' : 'bg-white text-gray-900'
                    }`
                  : "rounded-xl p-4"
              } text-sm border ${
