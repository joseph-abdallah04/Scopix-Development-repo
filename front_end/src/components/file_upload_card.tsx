      ...
      <div className="flex flex-row gap-16 items-start justify-center w-full">
        <div className="flex flex-col items-center justify-center gap-6">
          <div className={`w-[450px] h-[500px] border-2 border-dashed rounded-2xl p-6 flex flex-col items-center justify-center gap-4 transition-colors duration-300 ${
            ...
            <label className={`cursor-pointer flex flex-col items-center justify-center px-6 py-8 rounded-2xl transition-all duration-300 active:scale-95 group ${
              isDarkMode 
                ? 'hover:bg-blue-700' 
                : 'hover:bg-blue-600'
            }`}>
              <FiUpload className={`w-20 h-20 transition-colors duration-300 group-hover:text-white ${
                isDarkMode 
                  ? 'text-white' 
                  : 'text-gray-600'
              }`} />
              <span className={`mt-2 text-xl transition-colors duration-300 group-hover:text-white ${
                isDarkMode 
                  ? 'text-gray-400' 
                  : 'text-gray-500'
              }`}>{fileLabel}</span>
